...
function* sortRecipes() {
  try {
    const sortedRecipes = yield call(recipeAPI.sortRecipes);
    const normalizeData = normalize(sortedRecipes.data, arrayOfRecipes);

    yield put({
      type: constants.SORT_RECIPES_SUCCESS,
      payload: {
        all: normalizeData.result,
        byId: normalizeData.entities.byId || {}
      }
    ...
    yield put({
      type: constants.SORT_RECIPES_FAILED
    });
  }
}
...
export default function* recipeSaga() {
  yield all([
    ...
    takeLatest(constants.SORT_RECIPES, sortRecipes),
